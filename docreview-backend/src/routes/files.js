...
function getSupabase() {
  ...
  if (!url) throw new Error("Missing SUPABASE_URL (set in backend .env)");
  if (!key) throw new Error("Missing SUPABASE_SERVICE_ROLE_KEY (set in backend .env)");
...
function makeObjectPath(userId, originalName) {
  const ts = Date.now();
  const safe = (originalName || "file").replace(/[^\w.\-]+/g, "_").replace(/^_+/, "");
...
function toAbsoluteUrl(_req, u) { return u; }
...
router.get("/", authRequired, async (req, res) => {
  try {
    ...
    res.json(files.map(f => ({ ...f, absoluteUrl: toAbsoluteUrl(req, f.url) })));
  } catch (err) {
...
router.post("/", authRequired, upload.single("file"), async (req, res) => {
  ...
  try {
    const supabase = getSupabase(); // <= dibuat di sini
    const isPublic = req.body.isPublic === "true" || req.body.isPublic === true;

    let allowedEmails = [];
    const raw = req.body.allowedEmails;
    if (!isPublic) {
      if (Array.isArray(raw)) allowedEmails = raw;
      else if (typeof raw === "string" && raw.trim()) {
        try { const parsed = JSON.parse(raw); allowedEmails = Array.isArray(parsed) ? parsed : [raw]; }
        catch { allowedEmails = [raw]; }
      }
    ...
    const up = await supabase.storage.from(BUCKET).upload(objectPath, req.file.buffer, {
      contentType: req.file.mimetype || "application/octet-stream",
      upsert: false,
    });
    if (up.error) {
      console.error("Supabase upload error:", up.error.message);
    ...
    if (!isPublic && allowedEmails.length) {
      const users = await prisma.user.findMany({ where: { email: { in: allowedEmails } }, select: { id: true, email: true } });
      const foundIds = users.map(u => u.id);
      const foundEmails = new Set(users.map(u => u.email));
      const unknown = allowedEmails.filter(e => !foundEmails.has(e));
      if (unknown.length) return res.status(400).json({ error: `Unknown users: ${unknown.join(", ")}` });
      const uniqueIds = Array.from(new Set(foundIds.filter(uid => uid !== req.user.id)));
      if (uniqueIds.length) await prisma.fileAccess.createMany({ data: uniqueIds.map(uid => ({ fileId: file.id, userId: uid })) });
    }
...
router.patch("/:id/access", authRequired, async (req, res) => {
  try {
    ...
    const isPublic = req.body.isPublic === true || req.body.isPublic === "true";
    const updated = await prisma.file.update({ where: { id: file.id }, data: { isPublic } });
    ...
    if (!isPublic) {
      ...
      const users = await prisma.user.findMany({ where: { email: { in: cleaned } }, select: { id: true } });
...
router.delete("/:id", authRequired, async (req, res) => {
  try {
    ...
    if (f.ownerId !== req.user.id) return res.status(403).json({ error: "Forbidden" });

    const supabase = getSupabase(); // <= dibuat di sini
    const objectPath = extractObjectPathFromPublicUrl(f.url);
    if (objectPath) {
      const del = await supabase.storage.from(BUCKET).remove([objectPath]);
      if (del.error) console.warn("Storage remove warning:", del.error.message);
    } else {
